...
select i.name 'item_name', c.name 'category_name'
from item i
inner join category c on i.category_id = c.category_id
...
select i.name, i.price_per_unit, u.name
from item i
...
select c.first_name, c.last_name, e.first_name
from customer c
left outer join project p on c.customer_id = p.customer_id
-- left outer join project_employee pe on p.project_id = pe.project_id and pe.employee_id = 32
left outer join project_employee pe on p.project_id = pe.project_id and pe.employee_id = (
select e.employee_id
from employee e 
where e.first_name = 'Fleur' and e.last_name = 'Soyle'
)
left outer join employee e on e.employee_id = pe.employee_id and e.first_name = 'Fleur' and e.last_name = 'Soyle'
...
select c.name 'category', pc.name 'parent_category'
from category pc
left outer join category c on c.category_id = pc.parent_category_id;
...
select c.customer_id, c.first_name, c.last_name, p.description, pi.quantity, i.name, cat.name, u.name 
from customer c 
inner join project p on c.customer_id = p.customer_id and c.postal_code = 'L3K'
inner join project_item pi on p.project_id = pi.project_id 
